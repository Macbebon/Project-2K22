...
$id=  mysqli_real_escape_string($cid,$_REQUEST['customer_id']);
$sql="SELECT * FROM `customer` WHERE id=$id";
$result=  mysqli_query($cid,$sql) or die(mysqli_error($cid));
$rws=  mysqli_fetch_array($result);
?>
<?php
                        $delete_id=  mysqli_real_escape_string($cid,$_REQUEST['customer_id']);
                        if(isset($_REQUEST['submit2_id'])){
                            ...
                            mysqli_query($cid,$sql_drop) or die(mysqli_error($cid));
