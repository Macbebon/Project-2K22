...
$id=  mysqli_real_escape_string($cid,$_REQUEST['staff_id']);
$sql="SELECT * FROM `staff` WHERE id=$id";
$result=  mysqli_query($cid,$sql) or die(mysqli_error($cid));
$rws=  mysqli_fetch_array($result);
?>
<?php
                        $delete_id=  mysqli_real_escape_string($cid,$_REQUEST['staff_id']);
                        if(isset($_REQUEST['submit2_id'])){
                            $sql_delete="DELETE FROM `staff` WHERE `id` = '$delete_id'";
                            mysqli_query($cid,$sql_delete) or die(mysqli_error($cid));
...
<html>
    ...
    <?php include 'admin_navbar.php'?>
            ...
            <table align="center" >
                ...
                <tr>
                    <td>Email id</td>
                    <td><input type="text" name="edit_emailid" value="<?php echo $rws[8];?>" required=""/></td>
                </tr>
